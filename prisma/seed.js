...
async function main() {
  ...
  // Create profile and admin
  const adminPassword = await hashPassword('admin123');
  const profile = await prisma.profile.create({
    data: {
      name: 'Madhav Malhotra',
      slug: 'madhav-malhotra',
  ...
  const skills = await prisma.skill.createMany({
    data: [
      { name: 'JavaScript', category: 'Language', level: 95, profileId: profile.id },
      { name: 'TypeScript', category: 'Language', level: 90, profileId: profile.id },
      { name: 'React', category: 'Frontend', level: 95, profileId: profile.id },
      { name: 'Angular', category: 'Frontend', level: 85, profileId: profile.id },
      { name: 'Node.js', category: 'Backend', level: 90, profileId: profile.id },
      { name: 'Express.js', category: 'Backend', level: 88, profileId: profile.id },
      { name: 'PostgreSQL', category: 'Database', level: 85, profileId: profile.id },
      { name: 'MongoDB', category: 'Database', level: 80, profileId: profile.id },
      { name: 'Prisma', category: 'ORM', level: 90, profileId: profile.id },
      { name: 'Docker', category: 'DevOps', level: 75, profileId: profile.id },
    ],
  });
